...
<br>Телефон: <span id="boxberry-issue_point-phone">' . $point['Phone'] . '</span>, часы работы: <span id="boxberry-issue_point-work">' .
   ...
   data-order-id=""
    ...
    public function getBoxberryZip()
    {
        ...
        $client->setApiUrl($this->config->get('shipping_boxberry_api_url'));
        $city_code = null;
        if (!($zip = $this->model_boxberry_zip->getZip($this->session->data['shipping_address']['postcode'])) || !($expired = $zip['expired']) || (strtotime($expired) <= time())) {
            $zip_req = $client->getZipCheck();
            $zip_req->setZip($this->session->data['shipping_address']['postcode']);
            try {
                $zip = $client->execute($zip_req);
                if ($zip->getExpressDelivery()) {
                    $this->model_boxberry_zip->addZip(['zip' => $this->session->data['shipping_address']['postcode']]);
    ...
    protected function getIssuePointCost($address, $prepaid)
    {
        ...
        if (isset($this->session->data['boxberry_shipping_issue_point_id' . ($prepaid ? '_prepaid' : '')])
            ...
            if ($this->area == ''){
                foreach ($boxberry_cities as $city) {
                    if ($city->getName() == str_replace('ё','е',$address['city']) || $city->getName() == mb_strtoupper(mb_substr(str_replace('ё','е',$address['city']), 0, 1)) . mb_substr($address['city'], 1)) {
                ...
                if (!$fullAddress){
                    ...
                    {
                        /** var /Boxberry/Models/City $city */
                        if ($city->getName() == trim(mb_strtoupper($address['city'])) || $city->getName() == trim(mb_strtoupper(mb_substr($address['city'], 0, 1))) . trim(mb_substr($address['city'], 1))) {
                            $city_code = $city->getCode();
                            break;
                        }
                    }
    ...
    protected function getCourierDeliveryCost($address, $prepaid)
    {
        ...
        $deliveryCosts->setPaysum($prepaid ? 0 : $this->cart->getTotal());
        $deliveryCosts->setOrdersum($this->cart->getTotal());
        $deliveryCosts->setZip($address['postcode']);
        $deliveryCosts->setCms('opencart');
        $deliveryCosts->setUrl($_SERVER['HTTP_HOST']);
        $deliveryCosts->setVersion('1.0');
        $responseObject = $client->execute($deliveryCosts);
